...
expected_response = [
    ...
    ({"repo_code": {"repo": "https://example.org", "branch": "main"}}, 201),
    ({"repo_docs": {"repo": "https://example.org", "branch": "main"}}, 400),
    ({}, 400),
]
...
async def test_add_pipeline_for_assessment(mocker, client, body, expected):
    ...
    """
    mocker.patch(
        "openapi_server.controllers.default_controller._get_tooling_for_assessment",
        return_value=("a", "b"),
    )
    mocker.patch(
        "openapi_server.controllers.default_controller._add_pipeline_to_db",
    ...
    assert response.status == expected, "Response body is : " + (
        await response.read()
    ).decode("utf-8")
