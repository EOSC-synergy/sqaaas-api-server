...
class JePLUtils(object):
    ...
    def get_commands_script(
        ...
        if template_name in ["im_client", "ec3_client"]:
            ...
            if template_name in ["im_client"]:
                if im_config_file:
                    if im_config_file.endswith("radl"):
                        ...
                        _reason = "File <%s> not recognized as either TOSCA or RADL"
                else:
                    _reason = (
            ...
            if template_name in ["ec3_client"] and not ec3_templates:
                _reason = (
                    "No RADL templates provided for ec3_client: " "%s" % template_kwargs
                )
    ...
    def compose_files(cls, config_json, composer_json, report_to_stdout=False):
        ...
        config_data_list = cls.append_file_name("config", config_data_list)
        composer_data = cls.append_file_name("composer", [composer_data])[0]
    ...
    def get_files(file_type, gh_utils, repo, branch):
    ...
    def push_files(
        ...
        config_files_to_push_names = [
            file_dict["file_name"] for file_dict in config_files_to_push
        ]
        ...
        commands_scripts_to_push_names = [
            file_dict["file_name"] for file_dict in commands_scripts_to_push
        ]
        commands_scripts_to_remove_set = set(commands_scripts_from_repo).difference(
            set(commands_scripts_to_push_names)
        )
        ...
        logger.debug(
            ("GitHub repository <%s> created with the JePL file " "structure" % repo)
        )
    ...
    def get_composer_service(
        ...
        if dockerfile:
            ...
            context = "./" + context
            srv_data["build"] = {"context": context, "dockerfile": dockerfile}
            if build_args:
                srv_data["build"]["args"] = build_args
            logger.debug(
                "Dockerfile build context set for service <%s>: %s"
                % (name, srv_data["build"])
            )
        if not oneshot:
        ...
        if environment:
            ...
            logger.debug(
                "Setting environment for service <%s>: %s" % (name, environment)
            )
    ...
    def generate_stage_name(repo_name):
        ...
        logger.debug(
            "New counter-based repository name generated: " "<%s>" % stage_name
        )
        return stage_name
