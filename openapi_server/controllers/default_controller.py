...
ASSESSMENT_REPORT_LOCATION = config.get(
    "assessment_report_location", fallback=".report/assessment_output.json"
)
STATUS_BADGE_LOCATION = config.get(
    "status_badge_location", fallback=".badge/status_shields.svg"
)
...
async def _add_pipeline_to_db(body, branch_upstream=None, report_to_stdout=False):
    ...
    pipeline_repo_url = "/".join(
        [SUPPORTED_PLATFORMS[REPOSITORY_BACKEND], pipeline_repo]
    )
    logger.debug(
...
async def add_pipeline(
    request: web.Request, body, report_to_stdout=None
) -> web.Response:
    """Creates a pipeline.
...
async def _get_tooling_for_assessment(
    repositories, main_repo_key, user_requested_tools=[]
):
        ...
        for criterion_data in criteria_data_list:
                    ...
                    try:
                        ...
                        if account_tool_by_requirement_level:
                            logger.debug(
                                ...
                                "level). Reason: %s" % (tool_name, _reason)
                            )
                    finally:
                        logger.debug(
                ...
                if account_tool_by_requirement_level:
                    ...
                    if not lang_entry:
                        ...
                        for field_name in ["extensions", "filenames"]:
                            ...
                            if files_found:
                                logger.debug('files_found' + str(files_found))
                    ...
                    if account_tool:
                        logger.info(
                            "Tool added for the assessment after filtering: "
                            "%s" % tool_name
                        )
            ...
            if not toolset_for_reporting:
                _reason = (
                    ...
                    "property) in criterion: %s" % criterion_id
                )
                if criterion_has_required_level:
                    filtered_out_data = ctls_utils.format_filtered_data(
                        False, filtered_required_tools
                    )
                ...
                logger.info(
                    "Found %s tool/s for the assessment of criterion <%s>: "
                    "%s"
                    % (
...
async def _get_criteria_for_digital_object(repositories):
    ...
    relevant_criteria_data.append(
        {"repo": _repo, "criteria_data_list": criteria_data_list}
    )
    logger.debug(
...
def _validate_assessment_input(body):
    ...
    if repo_code:
        ...
        if repo_docs:
            ...
            _same_code_and_docs_repo = list(repo_code.values()) == list(
                repo_docs.values()
            )
    ...
    # FAIR: set 'repo' property to None to avoid git clone in _filter_tools()
        ...
        main_repo_key = "fair"
        repositories["fair"] = {"repo": None, "fair_tool": fair["fair_tool"]}
    else:
...
async def add_pipeline_for_assessment(
    ...
    if is_fair:
        ...
        for _repo_key, _repo_data in repositories.items():
            ...
            logger.debug(
                "Repository settings generated for repo <%s>: %s"
                % (_repo_data["url"], _repo_settings)
            )
...
async def get_pipeline_composer_jepl(request: web.Request, pipeline_id) -> web.Response:
    ...
    r = {"file_name": composer_data["file_name"], "content": composer_data["data_json"]}
...
async def get_pipeline_config_jepl(request: web.Request, pipeline_id) -> web.Response:
    ...
    r = [
        {"file_name": config_data["file_name"], "content": config_data["data_json"]}
...
@ctls_utils.debug_request
@ctls_utils.validate_request
async def get_pipeline_jenkinsfile_jepl(
    request: web.Request, pipeline_id
) -> web.Response:
    ...
    r = {"file_name": "Jenkinsfile", "content": jenkinsfile}
...
def _set_im_config_files_content(additional_files_to_commit, repo_url, repo_branch):
    ...
    for additional_file in additional_files_to_commit:
        if additional_file["file_data"]:
            ...
            _repo = {"repo": repo_url, "branch": repo_branch}
            _tech = (
                "ec3_client"
            ...
            additional_files_list.append(
                ctls_utils.add_image_to_im(
                    im_config_file, im_image_id, openstack_url, tech=_tech, repo=_repo
                )
            )
...
async def run_pipeline(
    ...
    if repo_url:
        ...
        try:
            ...
            _reason = "Could not access to repository: %s (branch: %s)" % (
                repo_url,
                repo_branch,
            )
        ...
        additional_files_list = _set_im_config_files_content(
            pipeline_data["data"]["additional_files_to_commit"], repo_url, repo_branch
        )
    else:
        _create_repo = False
        ...
        if not _repo:
            ...
            if not gh_utils.get_repo_content(pipeline_repo):
                ...
                _create_repo = True
                logger.debug(
                    "Removing repository as part of re-creation:" "%s" % pipeline_repo
                )
        ...
        if _create_repo:
            ...
            _repo = gh_utils.create_org_repository(
                pipeline_repo, branch=pipeline_repo_branch, include_readme=True
            )
        if not pipeline_repo_branch:
            pipeline_repo_branch = _repo.default_branch

    logger.debug(
        "Using pipeline repository <%s> (branch: %s)"
    ...
    db.update_entry(pipeline_id, pipeline_repo_branch=pipeline_repo_branch)
...
async def _update_status(pipeline_id, triggered_by_run=False, build_task=None):
    ...
    if not _pipeline_executed:
        _reason = (
            "Could not retrieve Jenkins job information: Pipeline <%s> has not yet ran"
            % pipeline_id
        )
        logger.error(_reason)
        raise SQAaaSAPIException(422, _reason)
    ...
    if jenkins_info["scan_org_wait"]:
        ...
        if _job_info.get("lastBuild", None):
            try:
                ...
                logger.info(
                    "Jenkins job build URL (after Scan Organization finished) obtained: %s"
                    % build_url
                )
                jenkins_info["build_info"].update(
                    {"url": build_url, "number": build_no, "status": "EXECUTING"}
                )
            ...
            while not build_data:
                ...
                if build_item_no:
                    ...
                    if build_data:
                        ...
                        build_status = "EXECUTING"
                        logger.info(
                            "Jenkins job build URL obtained for repository <%s>: %s"
                            % (pipeline_data["pipeline_repo"], build_url)
                        )
                    else:
                        logger.debug(
                            "Could not get build data from Jenkins queue item: %s"
                            % build_item_no
                        )
        ...
        if _status["result"]:
            ...
            if _status.get("queueId", None):
                build_status = "EXECUTING"
                logger.debug(
                    "Jenkins job queueId found: setting job status to <EXECUTING>"
                )
    logger.info(
        "Build status <%s> for job: %s (build_no: %s)"
        % (build_status, jk_job_name, build_no)
    )
...
async def get_pipeline_status(request: web.Request, pipeline_id) -> web.Response:
    ...
    logger.info(
        "Successfully obtained the current status of the " "pipeline <%s>" % pipeline_id
    )
    logger.debug(r)
...
async def _run_validation(criterion_name, **kwargs):
    ...
    if validator_name not in allowed_validators:
        ...
        try:
            ...
            validator_package_name = "-".join(["report2sqaaas-plugin", validator_name])
            out.update(
                {
...
async def _get_tool_from_command(tool_criterion_map, stdout_command):
    ...
    for tool_name, tool_data in tool_criterion_map.items():
        ...
        # For the matching tool process, let's consider the last one
        tool_cmd = tool_cmd_list[-1]
        if stdout_command.find(tool_cmd) != -1:
            matched_tool = tool_name
            logger.debug(
                "Matching tool <%s> found for stdout command <%s>"
                % (matched_tool, tool_cmd)
            )
            break
    if not matched_tool:
...
async def _validate_output(stage_data_list, pipeline_data):
    ...
    for stage_data in stage_data_list:
        ...
        commands_from_script = await _get_commands_from_script(
            stdout_command, pipeline_data["data"]["commands_scripts"]
        )
        ...
        if broken_data:
            ...
            logger.info(
                "Add broken criterion <%s> to filtered criteria list" % (criterion_name)
            )
            continue
...
async def get_output_for_assessment(request: web.Request, pipeline_id) -> web.Response:
    ...
    def _get_coverage(subcriteria):
        ...
        if total_subcriteria > 0:
            percentage_criterion = int(success_subcriteria * 100 / total_subcriteria)

        return (total_subcriteria, success_subcriteria, percentage_criterion)
    ...
    def _format_report():
        ...
        for criterion_name, criterion_output_data_list in output_data.items():
            ...
            for criterion_output_data in criterion_output_data_list:
                ...
                plugin_data = {"name": package_name, "version": package_version}
                ...
                tool_data = {
                    ...
                    "version": criteria_tools[criterion_name][tool].get(
                        "version", None
                    ),
                ...
                for subcriterion_id, subcriterion_data in subcriteria.items():
                    valid = all(
                        evidence["valid"] for evidence in subcriterion_data["evidence"]
                    )
        ...
        for _criterion, _data in criteria_filtered.items():
            _criteria_filtered[_criterion] = {"valid": False, "subcriteria": {}}
    ...
    def _get_criteria_per_badge_type(report_data):
        ...
        if FAIR_PREFIX in _criteria_list:
            logger.info(
                ("QC.FAIR criterion detected: considering only " "QC.FAIR subcriteria")
            )
    ...
    if criteria_fulfilled_map:
        ...
        try:
            ...
            _reason = (
                "Could not retrieve Jenkins job information: Pipeline has not ran yet"
            )
            logger.error(_reason)
            return web.Response(status=422, reason=_reason, text=_reason)
                ...
                for subcriterion, subcriterion_data in _subcriteria.items():
                    ...
                    if not _valid:
                        try:
                            ...
                            (
                                report_data_copy[criterion]["subcriteria"][
                                    subcriterion
...
async def create_pull_request(request: web.Request, pipeline_id, body) -> web.Response:
    ...
    supported_platform = ctls_utils.supported_git_platform(
        body.repo, platforms=SUPPORTED_PLATFORMS
    )
    if not supported_platform:
        _reason = (
            "Git platform <%s> is currently not supported for creating pull "
            "requests (choose between: %s)"
            % (supported_platform, SUPPORTED_PLATFORMS.keys())
        )
        logger.error(_reason)
        return web.Response(status=422, reason=_reason, text=_reason)
    ...
    logger.debug(
        "Target repository (base) path: %s (branch: %s)"
    ...
    if fork_created:
        ...
        logger.debug(
            "Branch <%s> created from head branch <%s>"
            % (source_branch_name, target_branch_name)
        )
    logger.debug(
        "Source repository (head) path: %s (branch: %s)"
        % (source_repo.full_name, source_branch_name)
    )
    # step 2: push JePL files
    #   Set IM config file content now that we know the remote repo URL
    additional_files_list = (
        _set_im_config_files_content(
            pipeline_data["data"]["additional_files_to_commit"], body.repo, body.branch
        )
        or []
    )
    ...
    pr_exists = [
        pr_data for pr_data in target_pr_data if pr_data["data"] == source_repo_data
    ]
    if pr_exists:
...
async def get_compressed_files(request: web.Request, pipeline_id) -> web.Response:
    ...
    config_yml_list = [
        (data["file_name"], data["data_yml"]) for data in config_data_list
    ]
    composer_yml = [(composer_data["file_name"], composer_data["data_yml"])]
    jenkinsfile = [("Jenkinsfile", jenkinsfile)]
    if commands_scripts:
        commands_scripts = [
            (data["file_name"], data["content"]) for data in commands_scripts
        ]
    ...
    response.headers.add("Content-Disposition", 'attachment; filename="sqaaas.zip"')
...
async def _badgeclass_matchmaking(pipeline_id, badge_type, criteria_fulfilled_list):
    ...
    for badge_category in BADGE_CATEGORIES:
        logger.debug(
            "Matching given criteria against defined %s criteria for %s"
            % (badge_category.upper(), badge_type.upper())
        )

        criteria_summary[badge_category] = {"to_fulfill": [], "missing": []}
        ...
        # Matchmaking
        missing_criteria_list = list(
            set(criteria_to_fulfill_list).difference(criteria_fulfilled_list)
        )
        criteria_summary[badge_category]["missing"] = missing_criteria_list
        if missing_criteria_list:
            logger.warn(
                "Pipeline <%s> not fulfilling %s criteria. Missing criteria: %s"
                % (pipeline_id, badge_category.upper(), missing_criteria_list)
            )
        else:
            logger.info(
                "Pipeline <%s> fulfills %s badge criteria!"
    ...
    if badge_awarded_badgeclass_name:
        logger.debug(
            "Badgeclass to use for badge issuance: %s" % badge_awarded_badgeclass_name
        )
...
async def _get_badge_share(badge_data, commit_url):
    ...
    dt = pandas.to_datetime(badge_data["createdAt"], format="%Y-%m-%dT%H:%M:%S.%fZ")
...
async def get_badge(request: web.Request, pipeline_id) -> web.Response:
    ...
    logger.debug("Badge data found for pipeline <%s>: %s" % (pipeline_id, badge_obj))
...
async def _get_tooling_metadata():
    ...
    logger.debug(
        (
            ...
            "%s)" % (tooling_repo_url, tooling_repo_branch, tooling_metadata_file)
        )
    )
    platform = ctls_utils.supported_git_platform(
    ...
    if platform in ["github"]:
        ...
        raise NotImplementedError(
            (
                "Getting tooling metadata from a non-Github "
    ...
    for lang, tools in criterion_data.items():
        ...
        for tool in tools:
            ...
            try:
                ...
                logger.warn("Cannot find data for tool <%s> (lang: %s)" % (tool, lang))
            if d:
...
async def _sort_tooling_by_criteria(tooling_metadata_json, criteria_id_list=[]):
    ...
    if criteria_id_list:
        ...
        logger.debug(
            "Considering all the supported criteria from tooling <%s>"
            % criteria_id_list
        )
    ...
    try:
        for criterion in criteria_id_list:
            ...
            criterion_data.update({"id": criterion, "tools": tooling_data})
...
async def get_criteria(
    request: web.Request, criterion_id=None, assessment=None
) -> web.Response:
    ...
    criteria_data_list = await _get_criteria(criteria_id_list, assessment=assessment)
...
async def _handle_badge_status(pipeline_id, pipeline_data, badge_status=None):
    ...
    if repo_settings:
        ...
        if badge_status != badge_status_previous:
            logger.debug(
                "Status badge changed from <%s> to <%s>"
            ...
            db.add_repo_settings(pipeline_id, repo_settings)
            logger.info(
                "New status badge updated in DB for pipeline <%s>: status <%s>"
            ...
            logger.info(
                "New status badge pushed to repository <%s>: status <%s>"
