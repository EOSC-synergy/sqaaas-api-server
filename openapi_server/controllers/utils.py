...
docker_credential_id = config.get_ci("docker_credential_id", fallback=None)
docker_credential_org = config.get_ci("docker_credential_org", fallback=None)
docker_credential_id = config.get_ci("docker_credential_id", fallback=None)
docker_credential_org = config.get_ci("docker_credential_org", fallback=None)
...
def upstream_502_response(r):
    ...
    return web.json_response(r, status=502, reason=_reason, text=_reason)
        ...
        for srv_name, srv_data in composer_data["services"].items():
            try:
                ...
                if registry_data["push"]:
                    try:
                        if not (registry_data["credential_id"] or docker_credential_id):
                            raise KeyError
                    except KeyError:
                        _reason = (
                            "Request to push Docker images, but no credentials "
                    ...
                    try:
                        ...
                        _reason = (
                            "Request to push Docker images, but no image name provided!"
                        )
                        logger.warning(_reason)
                        return web.Response(status=400, reason=_reason, text=_reason)
                    try:
                        if not srv_data["build"]:
                            ...
                            has_context = (
                                "context" in srv_data["build"].keys()
                                and srv_data["build"]["context"]
                            )
                            has_dockerfile = (
                                "dockerfile" in srv_data["build"].keys()
                                and srv_data["build"]["dockerfile"]
                            )
                            if not (has_context or has_dockerfile):
                                raise KeyError
                    except KeyError:
                        _reason = (
                            "Request to push Docker images, but no build data provided!"
                        )
                        logger.warning(_reason)
                        return web.Response(status=400, reason=_reason, text=_reason)
        # Tooling
...
class ProcessExtraData(object):
    ...
    def set_service_volume(service_data):
        ...
        try:
            ...
            service_data["volumes"] = [
                {"type": "bind", "source": "./", "target": "/sqaaas-build"}
            ]
            logger.debug(
    ...
    def set_tox_env(repo_checkout_dir, repos_data):
        ...
        if not "tox" in repos_data[repo_key].keys():
            ...
            if not testenv:
                testenv = ["ALL"]
            logger.debug(
                "Tox environment set (tox_file: %s, testenv: %s)" % (tox_file, testenv)
            )
    ...
    def load_tooling_repo(project_repos_mapping, config_json):
        ...
        if tooling_repo_url in list(project_repos_mapping):
            logger.debug(
                "Tool template repository <%s> already defined" % tooling_repo_url
            )
        else:
            repo_data = {}
            repo_data["name"] = get_short_repo_name(tooling_repo_url, include_host=True)
    ...
    def curate_service_image_properties(
        composer_json,
        service_name=None,
        tool={},
        criterion_name=None,
        ...
        logger.info(
            "Service <%s> image properties updated: %s"
            % (service_name, service_image_properties)
        )
    ...
    def set_tool_execution_command(
                ...
                if arg.get("repeatable", False):
                    if len(value_list) == 1:
                        if option_no_flag:
                            ...
                            return list(
                                map(
                                    lambda value: " ".join([option_no_flag, value]),
                                    value_list.split(),
                                )
                            )
        ...
        def process_args(args, cmd_list=[]):
            ...
            for arg in args:
                ...
                if not flag:
                    cmd_list.extend(
                        process_value(arg, commands_builder=commands_builder)
                    )
                if arg.get("args", []):
        ...
        if tool in list(tool_map):
            ...
            tool_map[tool_name] = {"commands": cmd}
        # If applicable, print the generated report to stdout
        if report_to_stdout:
            report_file = tool.get("includes_report", None)
            if report_file:
                logger.debug(
                    "Adding `cat` command (last step) to print generated report to stdout"
                )
    ...
    def set_config_when_clause(config_json):
        ...
        for criterion_name, criterion_data in config_json["sqa_criteria"].items():
            ...
            if "when" in criterion_data.keys():
                ...
                config_json_when["sqa_criteria"] = {criterion_name: criterion_data_copy}
                when_data = criterion_data_copy.pop("when")
                config_data_list.append(
                    {"data_json": config_json_when, "data_when": when_data}
                )
                config_json_no_when["sqa_criteria"].pop(criterion_name)
            else:
                config_json_no_when["sqa_criteria"][
                    criterion_name
                ] = criterion_data_copy

        if config_json_no_when["sqa_criteria"]:
            config_data_list.append(
                {"data_json": config_json_no_when, "data_when": None}
            )
...
def process_extra_data(config_json, composer_json, report_to_stdout=False):
    ...
    commands_script_list = []
    tool_criteria_map = {}
    service_images_curated_list = (
        []
    )  # services processed by curate_service_image_properties()
            ...
            for repo in repos_old:
                ...
                if not service_name:
                    if not tooling_repo_is_loaded:
                        logger.debug(
                            "Service name is not defined: adding tooling repository to config.yml"
                        )
                        ProcessExtraData.load_tooling_repo(
                            project_repos_mapping, config_json
                        )
                        tooling_repo_is_loaded = True
                    else:
                        logger.debug(
                            "Service name is not defined: tooling repository already loaded in config.yml"
                        )
                ...
                try:
                    ...
                    stage_name = " ".join(
                        [project_repos_mapping[repo_url]["name"], tool["name"]]
                    )

                if stage_name in list(repos_new):
                    stage_name_last = stage_name_mapping.get(stage_name, stage_name)
                    stage_name_new = JePLUtils.generate_stage_name(stage_name_last)
                ...
                for arg in tool.get("args", []):
                    ...
                    if arg["type"] in ["optional"]:
                        ...
                        if option.find("jenkins-credential-id") != -1:
                            creds["id"] = arg["value"]
                        elif option.find("jenkins-credential-variable") != -1:
                    ...
                    if "commands" in repo.keys():
                        ...
                        for arg in tool.get("args", []):
                            if arg.get("id", None):
                                ...
                                # comma-separated
                                if type(_value) in [str] and not arg.get(
                                    "value_has_commas", False
                                ):
                        ...
                        if tool_has_template in ["im_client", "ec3_client"]:
                            ...
                            template_kwargs.update(config.get_service_deployment(iaas))
        ...
        if "image" in list(srv_data):
            ...
            if "registry" in srv_data["image"].keys():
                ...
                if registry_data["push"]:
                    ...
                    logger.debug(
                        "Setting JPL_DOCKERPUSH environment value to <%s>" % srv_push
                    )
                    credential_id = None
                    if registry_data.get("credential_id", None):
                        credential_id = registry_data["credential_id"]
                        logger.debug(
                            "Using custom Jenkins credentials: %s" % credential_id
                        )
                    else:
                        credential_id = docker_credential_id
                        use_default_dockerhub_org = True
                        logger.debug(
                            "Using catch-all Jenkins credentials: %s" % credential_id
                        )
                    try:
...
def get_short_repo_name(repo_url, include_host=False):
    ...
    if include_host:
        ...
        if url_parsed.port:
            host = ":".join([host, str(url_parsed.port)])
...
def get_language_entry(lang):
    ...
    with open(language_metadata_file) as yaml_file:
        try:
            ...
            _reason = "Could not load <%s> file: %s" % (language_metadata_file, str(e))
            logger.error(_reason)
        else:
...
def find_files_by_language(field, value, repo, path="."):
    ...
    if field in ["extensions"]:
        ...
        logger.warn(("Language field <%s> (from languages.yml) not supported!" % field))
    if files_found:
        logger.debug(
            "Files found in path matching required %s: %s" % (field, files_found)
        )
...
def get_registry_from_image(image_name):
    ...
    if url_parsed.scheme:
        registry_url = "://".join([url_parsed.scheme, registry_url])
    if url_parsed.port:
        registry_url = ":".join([registry_url, str(url_parsed.port)])
...
def add_image_to_im(
    ...
    def _add_image_id_to_radl(data, image_id):
        if tech in ["ec3_client"]:
            ...
            class include(IM_RADL.FeaturedAspect):
                def check(self, r):
                    self.check_simple(
                        dict(template=(str, lambda x, _: len(x.value.strip()))), r
                    )
            ...
            class description(IM_RADL.FeaturedAspect):
                def check(self, r):
                    self.check_simple(
                        dict(short=(str, None), content=(str, None), kind=(str, None)),
                        r,
                    )
    ...
    if Path(im_config_file_abspath).exists():
        with open(Path(im_config_file_abspath), "r") as f:
            if im_config_file.endswith(".radl"):
                ...
                _reason = "IM config file type not supported: %s" % im_config_file
    else:
    ...
    return {"file_name": im_config_file, "file_data": data}
...
def get_credential_data(credential_id, pipeline_data):
    ...
    if not credential_data:
        logger.warning(
            "Could not find any credential data for credential: " "%s" % credential_id
        )
...
def get_status_badge(status, digital_object_type):
    ...
    """
    status_badge_map = {
        "not assessed": {"value": "not assessed", "default_color": "skyblue"},
